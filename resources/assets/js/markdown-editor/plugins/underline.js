...
export default function underlinePlugin(context) {
  ...
  return {
    markdownCommands: {
        underline(_options, context, dispatch) {
            ...
            if (textContent.startsWith(openTag) && textContent.endsWith(closeTag)) {
                replace = textContent
                    .substr(openTag.length)
                    .substr(0, textContent.length - openTag.length - closeTag.length);
            } else {
