...
<div {{ $attributes->class('accordion group') }} x-data="{
    openPanel: false,
    toggle: function() {
        this.openPanel = !this.openPanel;
        @if ($onToggle) ({{ $onToggle }}).call(this); @endif
    },
}" :class="{ 'accordion-open': openPanel }">
    <dl>
        <div @class([
            $containerClass,
            'border-t border-theme-secondary-300 md:border-2 md:border-theme-primary-100 md:rounded-xl' =>
                $dark === false && $border,
        ])>
            <dt>
                <button @click="toggle" @class([
                    ...
                    @if ($buttonOpenClass) :class="{ '{{ $buttonOpenClass }}': openPanel }" @endif>
                    ...
                    <div
                        class="transition-default @unless ($borderlessIcon)
                        border-2 border-theme-primary-100 group-hover:border-theme-primary-400
                        @endunless flex h-8 w-8 items-center justify-center rounded-lg">
                        <span
                            :class="{
                                '{{ $iconOpenClass }}': openPanel,
                                '{{ $iconClosedClass }}': !openPanel
                            }"
                            @class(['transition duration-150 ease-in-out transform', $iconClass])>
            ...
            ]) x-show="openPanel" x-transition.opacity x-cloak>
                {{ $slot }}
            </dd>
        </div>
    </dl>
