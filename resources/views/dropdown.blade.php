@props([
    ...
    'dropdownContentClasses' =>
        'bg-white rounded-xl shadow-lg dark:bg-theme-secondary-800 dark:text-theme-secondary-200',
...
<div @if ($withPlacement) x-data="Dropdown.setup('{{ $dropdownProperty }}', {
            @if ($onClose)
                onClosed: ({{ $onClose }}), @endif
    @if ($onOpen) onOpened: ({{ $onOpen }}), @endif placement: '{{ $withPlacement }}' ,
    @if ($placementFallbacks) placementFallbacks: {{ json_encode($placementFallbacks) }}, @endif })"
x-init="init" @elseif ($initAlpine) x-data="{ {{ $dropdownProperty }}: false }" x-init="$watch('{{ $dropdownProperty }}', (expanded) => {
    if (expanded) {
        $nextTick(() => {
            @if($onOpen)
            $nextTick(() => {
    ...
    @unless ($withoutButton)
        <div>
            <button type="button"
                :class="{
                    '{{ $buttonClassExpanded }}': {{ $dropdownProperty }},
                    '{{ $buttonClassClosed }}': !
                        {{ $dropdownProperty }}
                }"
                class="dropdown-button transition-default {{ $buttonClass }} flex items-center focus:outline-none"
    ...
    <div x-show="{{ $dropdownProperty }}" x-transition:enter="transition ease-out duration-100"
        ...
        <div class="{{ $dropdownContentClasses }}" x-cloak>
            <div @class($contentClass)
                @if ($closeOnClick ?? true) @click="{{ $dropdownProperty }} = !{{ $dropdownProperty }}" @endif>
                {{ $slot }}
            </div>
        </div>
    </div>
