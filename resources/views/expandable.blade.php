...
<ol data-expandable x-data="{ expanded: false }" :class="{ 'show-all': expanded }"
    style="--expandable-total-count: {{ $total }};{{ $style }}" x-init="$watch('expanded', (expanded) => {
        if (expanded) {
            $nextTick(() => {
    ...
    @empty($showMore)
        <button data-trigger class="{{ $triggerClass }}" @click="expanded = !expanded"
            @isset($triggerDusk)
                dusk="{{ $triggerDusk }}"
            @endisset>
