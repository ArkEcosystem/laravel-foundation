...
<footer {{ $attributes->class('border-t border-theme-secondary-800')->class($backgroundColor) }}>
    <div class="{{ $wrapperClass }} mx-auto items-center justify-between space-y-4 md-lg:flex md-lg:space-y-0">
        <div class="break-words text-sm font-semibold leading-6">
            <span>
                {{ date('Y') }}
                &copy;
                ...
                @elseif ($copyrightName && $copyrightUrl)
                    <a href="{{ $copyrightUrl }}" @class(['transition-default', $copyrightClass])>
                        {{ $copyrightName }}
                    </a>
                @elseif (isset($creator['label']) || is_string($creator))
                    <span>
                        @lang ('ui::pages.footer.made_with_love') <a href="{{ $creator['url'] ?? url('/') }}"
                            class="{{ $linkClass }}"
                            @if ($creator['newWindow'] ?? false) target="_blank" rel="noopener" @endif>{{ is_string($creator) ? $creator : $creator['label'] }}</a>
                    </span>
                @endif
            </span>
            ...
            @if ($isArkProduct)
                <span class="mx-0.5">|</span>
                <div class="inline">
                    <x-ark-icon name="networks.ark-square" class="ark-logo-red -mt-1 mr-1 inline-block" />
