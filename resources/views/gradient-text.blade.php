@props(['from', 'via' => null, 'to', 'direction' => 'right', 'animated' => false, 'animationSpeed' => '15s'])

<span
    style="
        @if ($animated) @if ($via === null)
            ...
            @else
                --tw-gradient-from: {{ $from }};
                --tw-gradient-to: {{ $from }};
                --tw-gradient-stops: var(--tw-gradient-from), {{ $via }}, {{ $to }}, var(--tw-gradient-to, {{ $from }}); @endif

            animation: move-bg {{ $animationSpeed }} infinite linear;
@else
@if ($via === null) --tw-gradient-from: {{ $from }};
                --tw-gradient-to: {{ $to }};
                --tw-gradient-stops: var(--tw-gradient-from), var(--tw-gradient-to, {{ $to }});
            @else
                --tw-gradient-from: {{ $from }};
                --tw-gradient-to: {{ $to }};
                --tw-gradient-stops: var(--tw-gradient-from), {{ $via }}, var(--tw-gradient-to, {{ $to }}); @endif
        @endif
    "
    {{ $attributes->class([
