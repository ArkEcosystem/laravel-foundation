...
<div @class($class) {{ $attributes->only(':class') }}>
    <div @class([
        ...
        <div class="absolute flex h-5 items-center">
            <input id="{{ $id ?? $name }}" name="{{ $name }}" type="checkbox"
                class="form-checkbox input-checkbox focus-visible:ring-2 focus-visible:ring-theme-primary-500"
                @unless ($noLivewire)
                    @if ($deferred)
        ...
        <div @class([
            ...
            <label for="{{ $id ?? $name }}" @class($labelClasses)
                @if ($alpineLabelClass) :class="{{ $alpineLabelClass }}" @endif>
