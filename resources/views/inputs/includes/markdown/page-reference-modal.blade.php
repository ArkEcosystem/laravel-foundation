<x-ark-js-modal init name="pageReferenceModal" :title="trans('ui::markdown.modals.pageReference.title')" width-class="max-w-lg">
    @slot('description')
        <form id="pageReferenceModalForm" @submit.prevent="Livewire.dispatch('pageReferenceModal', $event)" class="space-y-4">

            <x-ark-input type="text" name="url" :label="trans('ui::markdown.modals.pageReference.form.url')" class="w-full" />
        </form>
    @endslot
