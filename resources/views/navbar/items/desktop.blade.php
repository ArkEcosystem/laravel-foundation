@props([
    ...
    'invertedCurrentRouteTextColor' =>
        'text-white inverted:text-theme-secondary-900 inverted:hover:text-theme-secondary-900 dark:text-theme-secondary-400',
    'invertedRouteTextColor' =>
        'text-theme-primary-100 inverted:text-theme-secondary-700 hover:text-white inverted:hover:text-theme-secondary-900 dark:text-theme-secondary-400',
    'invertedRouteDropdownTextColor' => 'text-theme-primary-600',
    'invertedRouteDropdownOpenTextColor' => 'text-theme-primary-600',
    'invertedCurrentRouteBorderColor' =>
        'border-theme-primary-200 inverted:border-theme-primary-600 hover:border-theme-primary-200 inverted:hover:border-theme-primary-600 focus-visible:border-b-0',
    'invertedRouteBorderColor' =>
        'border-transparent hover:border-theme-primary-400 inverted:hover:border-theme-secondary-300 focus-visible:border-b-0',
])
...
@if (is_array($navigation))
    <div class="{{ $breakpointClasses }} hidden h-full items-center">
        @foreach ($navigation as $navItem)
            @isset($navItem['children'])
                <div class="relative h-full">
                    <button x-ref="menuDropdownButton" @class([
                        ...
                        @click="toggleDropdown('{{ $navItem['label'] }}')" @click.outside="closeIfBlurOutside"
                        aria-haspopup="true" aria-controls="{{ $menuDropdownId }}"
                        x-bind:aria-expanded="openDropdown === '{{ $navItem['label'] }}'">
                        <span class="flex items-center justify-center"
                            :class="{ 'text-theme-primary-600': openDropdown === '{{ $navItem['label'] }}' }">
                            <span class="sr-only">
                        ...
                        <x-ark-chevron-toggle is-open="openDropdown === '{{ $navItem['label'] }}'" class="ml-2"
                    ...
                    <div x-ref="menuDropdown" id="{{ $menuDropdownId }}"
            ...
            @else
                @php
                    $isCurrentRoute =
                        array_key_exists('route', $navItem) &&
                        optional(Route::current())->getName() === $navItem['route'];

                    if (array_key_exists('active', $navItem)) {
                        $isCurrentRoute = $navItem['active'];
                    }
                @endphp
                <a @if (array_key_exists('href', $navItem)) href="{{ $navItem['href'] }}"
                    @else
                        href="{{ route($navItem['route'], $navItem['params'] ?? []) }}" @endif
                    @if (array_key_exists('attributes', $navItem)) @foreach ($navItem['attributes'] as $attribute => $attributeValue)
                    ...
                    @class([
                        ...
                        'focus-visible:border-b-0 focus-visible:pt-0 focus-visible:-mt-px' =>
                            $isCurrentRoute && !$inverted,
