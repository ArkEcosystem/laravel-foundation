...
<div class="flex">
    <div @class(['w-1 -mr-1 z-10', 'bg-theme-primary-600' => $isCurrent])></div>

    @unless ($disabled)
        <a href="{{ $route ? route($route, $routeParams) : $href }}" @class([
        ...
        @else
            <div class="w-full cursor-default px-8 py-4 font-semibold text-theme-secondary-500"
                dusk="navbar-item-{{ Str::slug($label) }}">
            @endunless
                ...
                <div class="flex flex-1 flex-col space-y-2">
                    <span @class([
                        ...
                        @if ($external && !$hideExternalIcon)
                            <x-ark-icon name="arrows.arrow-external" size="sm" class="text-theme-primary-500" />
                        @endif
                    </span>
