...
<x-ark-dropdown wrapper-class="whitespace-nowrap md:relative" :dropdown-classes="Arr::toCssClasses(['w-full md:w-auto mt-4', 'min-w-63' => $showDarkMode, $profileMenuClass ?? null])"
    ...
    <x-slot name="button">
        <span class="avatar-wrapper relative inline-block">
            @isset($identifier)
                <x-ark-avatar :identifier="$identifier" :show-identifier-letters="$showIdentifierLetters ?? false" class="-m-1 h-12 w-12 rounded-xl md:h-13 md:w-13"
                    x-bind:class="{ 'border-theme-primary-600': dropdownOpen }" />
            @else
                <div class="h-10 w-10 overflow-hidden rounded-xl border-2 border-transparent md:h-11 md:w-11">
    ...
    @foreach ($profileMenu as $menuItem)
        ...
        @if ($menuItem['isPost'] ?? false)
            <form method="POST" action="{{ route($menuItem['route']) }}">
                ...
                <button type="submit" class="dropdown-entry focus-visible:rounded focus-visible:ring-inset"
