...
@php
    $generateClassList = fn($variable, $classList) => $variable instanceof Illuminate\View\ComponentSlot
...
<section {{ $attributes }}>
    <div @class([
        $wrapperClass,
        'flex md:items-center flex-col md:flex-row-reverse' =>
            $inverted && !$stacked,
        'flex md:items-center flex-col md:flex-row' => !$inverted && !$stacked,
    ])>
        <div class="{{ $stacked ? 'sm:text-center' : ($image ? 'md:w-1/2' : 'md:w-full') }} mb-8 w-full md:mb-0">
