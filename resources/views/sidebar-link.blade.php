@props([
    ...
    'attrs' => [],
    'itemClass' =>
        'text-theme-secondary-900 hover:text-theme-primary-600 dark:text-theme-secondary-200 dark:hover:text-theme-primary-600',
    'activeItemClass' =>
        'dark:bg-theme-secondary-800 dark:text-theme-secondary-200 text-theme-primary-600 bg-theme-primary-100',
...
<div class="flex">
    ...
    <a @if ($href) href="{{ $href }}"
        ...
        @foreach ($attrs as $attribute => $attributeValue)
            {{ $attribute }}="{{ $attributeValue }}" @endforeach
        {{ $attributes->except(['href', 'class', 'dusk']) }}>

        @if ($icon && $iconAlignment === 'left')
            <x-ark-icon :class="Arr::toCssClasses([
