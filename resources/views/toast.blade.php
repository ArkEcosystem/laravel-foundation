...
<div role="alert" aria-live="polite"
    ...
    <span @class([
        ...
        <span @class(['text-sm font-semibold', 'sm:hidden' => !$alwaysShowTitle])>
            {{ $title ?? trans('ui::toasts.' . $type) }}
        </span>
    </span>

    <div @class(['toast-body', 'mr-4' => !$hideSpinner])>
        @if ($message)
            {{ $message }}
        @else
            {!! $slot !!}
    ...
    <button @if ($wireClose) wire:click="{{ $wireClose }}" @endif
        @if ($alpineClose) @click="{{ $alpineClose }}" @endif type="button" @class([
            'toast-button' => !$alwaysShowTitle,
            'toast-button__titled' => $alwaysShowTitle,
        ])
        @if ($target) wire:loading.remove
